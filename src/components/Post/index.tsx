...
export function Post({
  ...
  const { register, handleSubmit, watch, reset } = useForm<CreateCommentFormData>({
    ...
    <PostContainer>
      ...
      {commentField && (
        <>
          <CommentForm onSubmit={handleSubmit(addNewComment)}>
            <textarea placeholder="Escreva um comentário" {...register('content')} />
            <CommentButton type="submit" hidden={!commentFieldChange}>
              Comentar
            </CommentButton>
          </CommentForm>
      ...
      <PostFooter>
        {(userLikedPosts ?? []).includes(_id) ? (
          <UnlikeButton onClick={handleSetPostAsLiked}>
            <ThumbsUp size={24} weight="fill" /> Like
          </UnlikeButton>
        ) : (
          <LikeButton onClick={handleSetPostAsLiked}>
            <ThumbsUp size={24} weight="fill" /> Like
          </LikeButton>
        )}
        <p>{numberOfLikes}</p>
